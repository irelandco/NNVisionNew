...
public class ThreeLayerNet {
    ...
    public void train(double[][] inputs, double[][] outputs, int numberOfTrainingIterations) {
        for (int i = 0; i < numberOfTrainingIterations; ++i) {
            ...
            if(i % 10 == 0 || i == numberOfTrainingIterations - 1){
                diff = 0;
                int yes = 0;

                for (int j = 0; j < inputs.length; j++) {
                    think(new double[][] {inputs[j]});
                    String string = String.format("%5.3f", getOutput()[0][0]) + String.format(" / %5.3f", outputs[j][0]) + String.format("   %5.3f", getOutput()[0][0]);
                    if (outputs[j][0] == 1) {
                        string = string + " Y";
                    }
                    ...
                    if (j > -1) {
                        diff += Math.abs(outputs[j][0] - getOutput()[0][0]);

                        if (Math.abs(outputs[j][0] - getOutput()[0][0]) < 0.1) yes++;
                    }
                }
                ...
                for (int j = 0; j < testInputs.length; j++) {
                    ...
                    String string = String.format("%5.3f", getOutput()[0][0]) + String.format(" %5.3f", getOutput()[0][0]) + String.format(" %5.3f", getOutput()[0][0]);
        ...
        try {
            PrintWriter out = new PrintWriter("Diffs.txt");
            for (Double d : diffs) out.println(d);
            out.close();
    ...
    public void exportNet() {
        ...
        try {
            out = new PrintWriter("ImageNet.txt");
        } catch (FileNotFoundException e) {
